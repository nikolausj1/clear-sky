...
> **Revision Notes (2026-07-17).** Justin approved the "Tonight's Sky" feature (full scope) after v1.0 shipped to his device: a card at the bottom of the Forecast screen with moon phase/rise/set, naked-eye planet visibility (on-device Meeus ephemerides), a daily sky-almanac line, aurora likelihood (NOAA SWPC), and ISS visible passes (Celestrak TLE + on-device SGP4). This amends the former "zero runtime network calls other than WeatherKit" rule: three additional endpoint families are sanctioned — NOAA SWPC (aurora + solar activity data), Celestrak (ISS orbital elements), and The Space Devs' Launch Library 2 (rocket launch schedule; anonymous tier, aggressively cached to respect its ~15 req/hr limit) — all free, keyless public services with on-disk caching and graceful offline degradation (sky rows render "—" when unreachable; on-device rows always render). No accounts, no backend, no runtime AI, no analytics — unchanged. **(2026-07-17, later):** Justin approved the full space-theme pivot — app renamed **Zenith**, and a third **Space tab** (Forecast | Space | Rankings) hosts upcoming rocket launches (Launch Library 2), solar activity (NOAA SWPC), and a 30-day sky-events calendar, keeping the Forecast's Tonight's Sky card compact. **(2026-07-18):** The dry-wit voice is retired app-wide (Justin's decision). The written voice is now the "Observatory Guide" register: clear, warm, factual, explanatory — the app's second hook is its sky intelligence, not humor. Section 6's canonical six wit lines no longer govern; the phrase bank remains deterministic and ≥8 variants per key. **(2026-07-18, night-first expansion):** The hero is a deliberate COMPOSITE of tonight (not a live sky view): every notable object/event of the night rendered together as a visual cheat sheet, always labeled "A look at tonight's sky." Notifications non-goal amended: exactly TWO opt-in local notifications are sanctioned — "ISS pass in 10 minutes" and "Aurora storm tonight" (G3+ forecasts only) — both default-off, no others. The Space Devs' image CDN (astronaut profile images) is sanctioned as part of the existing Launch Library 2 endpoint family. Widgets graduate from deferred to in-scope.
...
- Runtime network calls limited to exactly four sanctioned endpoint families: WeatherKit, NOAA SWPC (aurora + solar), Celestrak (ISS TLE), and Launch Library 2 (launches) - verified by inspecting network traffic during normal use; no analytics, no ad calls, no other endpoints. (Amended 2026-07-17 for Tonight's Sky; see Revision Notes.)
...
- No network call is made at runtime other than to the four sanctioned endpoint families - WeatherKit, NOAA SWPC, Celestrak, Launch Library 2 (verified by traffic inspection). Sky rows degrade to "—" offline; on-device sky rows (moon, planets, almanac) render regardless of connectivity.
